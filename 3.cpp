...
struct rw1{
	string nama, kota_tujuan, kota_asal, pax, waktu, total, username;
}riwayat_f[1024];
// Riwayat
...
void riwayat_tranksasi_read_f(string username)
{
	ifstream riwayat0("Data/Log-F/riwayat-username.log");
	ifstream riwayat1("Data/Log-F/riwayat-nama.log");
	ifstream riwayat2("Data/Log-F/riwayat-kota_asal.log");
	ifstream riwayat3("Data/Log-F/riwayat-kota_tujuan.log");
	ifstream riwayat4("Data/Log-F/riwayat-pax.log");
	ifstream riwayat5("Data/Log-F/riwayat-waktu.log");
	ifstream riwayat6("Data/Log-F/riwayat-total.log");
	
	for(int k=1;k<1024;k++)
	{
		getline(riwayat0, riwayat_f[k].username);
		getline(riwayat1, riwayat_f[k].nama);
		getline(riwayat2, riwayat_f[k].kota_asal);
		getline(riwayat3, riwayat_f[k].kota_tujuan);
		getline(riwayat4, riwayat_f[k].pax);
		getline(riwayat5, riwayat_f[k].waktu);
		getline(riwayat6, riwayat_f[k].total);
	}
	...
	for(int i=1;i<1024;i++)
	{
		if(riwayat_f[i].username==username)
		{
			cout<<"Nama tamu   : "<<riwayat_f[i].nama<<endl;
			cout<<"Kota asal   : "<<riwayat_f[i].kota_asal<<endl;
			cout<<"Kota Tujuan : "<<riwayat_f[i].kota_tujuan<<endl;
			cout<<"PAX         : "<<riwayat_f[i].pax<<endl;
			cout<<"Waktu       : "<<riwayat_f[i].waktu<<endl;
			cout<<"Total Biaya : "<<riwayat_f[i].total<<endl<<endl;
		}
	}
...
void riwayat_tranksasi_write_f(string nama, string kota_tujuan, string kota_asal, int pax, string waktu, int total, string username)
{
	ofstream riwayat0("Data/Log-F/riwayat-username.log", ios::app);
	ofstream riwayat1("Data/Log-F/riwayat-nama.log", ios::app);
	ofstream riwayat2("Data/Log-F/riwayat-kota_asal.log", ios::app);
	ofstream riwayat3("Data/Log-F/riwayat-kota_tujuan.log", ios::app);
	ofstream riwayat4("Data/Log-F/riwayat-pax.log", ios::app);
	ofstream riwayat5("Data/Log-F/riwayat-waktu.log", ios::app);
	ofstream riwayat6("Data/Log-F/riwayat-total.log", ios::app);
	
	riwayat0 << username <<endl;
	riwayat1 << nama <<endl;
	riwayat2 << kota_asal <<endl;
	riwayat3 << kota_tujuan <<endl;
	riwayat4 << pax <<endl;
	riwayat5 << waktu <<endl;
	riwayat6 << total <<endl;
...
void cari_f(string username)
{
	...
	riwayat_tranksasi_write_f(nama,data_f.wilayah[i][0],data_f.wilayah[i][1],pax,data_f.waktu[i][0],harga,username);
	
	system("pause");
}

void flight(string username)
{
	int pilihan;
	data_flight();
...
void main_order(int pilihan, string username)
{
	...
	switch(pilihan)
	{
		...
		case 3 : {
			...
			cout<<"+==========================+"<<endl;
			cout<<"Pilihan : ";cin>>kol;
			if(kol == 1) riwayat_tranksasi_read(username);
			else riwayat_tranksasi_read_f(username);
			break;
		}
		default : cout<<"Masukan Salah !!";getch();system("cls");break;
	}
